...
#include "xenstore_client_internal.h"
...
static struct xenstore_client xs_client;
/*
 ...
 */
static K_KERNEL_STACK_DEFINE(xs_workq_stack, CONFIG_XEN_STORE_CLIENT_WORKQ_STACK_SIZE);
...
{
	struct xenstore_client *xs = &xs_client;
...
static int read_frame_payload(struct xenstore_client *xs, size_t avail)
{
	...
	ret = validate_frame_payload(xs, hdr);
	if (ret < 0) {
		return fail_frame_payload(xs, hdr, ret);
	}

	ret = read_available_payload(xs, hdr, avail);
	if (ret < 0) {
		if (ret == -EAGAIN) {
			return ret;
		}
...
{
	uint32_t id = (atomic_inc(&xs_client.next_req_id) & UINT32_MAX);

	/* id=0 is reserved for watch notification */
	if (id == 0) {
		id = (atomic_inc(&xs_client.next_req_id) & UINT32_MAX);
	}
...
int xs_init(void)
{
	...
	if (is_ready(&xs_client)) {
		err = 0;
		goto out;
	}

	atomic_set(&xs_client.next_req_id, 1);
	xs_client.workq_priority = CONFIG_XEN_STORE_CLIENT_WORKQ_PRIORITY;

	k_work_init(&xs_client.event_work, response_worker);
	k_work_init_delayable(&xs_client.retry_work, response_retry_worker);
	k_mutex_init(&xs_client.req_mutex);

	if (!xs_client.workq_started) {
		k_work_queue_init(&xs_client.workq);
		k_work_queue_start(&xs_client.workq, xs_workq_stack,
				   K_THREAD_STACK_SIZEOF(xs_workq_stack), xs_client.workq_priority,
				   &qcfg);
		xs_client.workq_started = true;
	}

	sys_slist_init(&xs_client.notify_list);
	sys_slist_init(&xs_client.pending_responses);
	memset(xs_client.timed_out_req_ids, 0, sizeof(xs_client.timed_out_req_ids));
	...
	xs_client.domint = (struct xenstore_domain_interface *)vaddr;

	while (true) {
		size_t avail;

		err = response_ring_available(&xs_client, &avail);
		if (err < 0) {
			teardown_transport(&xs_client);
			goto out;
		}
		...
		(void)response_ring_read(&xs_client, NULL, avail);
	}

	err = bind_event_channel(xs_client.local_evtchn, response_evtchn_cb, &xs_client);
	if (err) {
	...
	xs_client.evtchn_bound = true;

	err = unmask_event_channel(xs_client.local_evtchn);
	if (err) {
...
{
	k_spinlock_key_t key = k_spin_lock(&xs_client.lock);

	xs_client.default_timeout = tout;

	k_spin_unlock(&xs_client.lock, key);
}
...
static k_timeout_t xs_default_timeout_get(void)
{
	...
	tout = xs_client.default_timeout;
...
struct xs_watcher *xs_watcher_register(xs_watch_cb cb, void *param)
{
	...
	w->registered = false;
	w->callback_running = false;
	w->release_after_callback = false;

	key = k_spin_lock(&xs_client.lock);

	if (!xs_client.initialized) {
		k_spin_unlock(&xs_client.lock, key);
		k_free(w);
		return NULL;
	}

	sys_slist_append(&xs_client.notify_list, &w->node);
	/*
	 * A watcher registered while another watch event is being dispatched must
	 * not receive that already in-progress event.
	 */
	w->dispatch_gen = xs_client.watch_dispatch_gen;
	w->registered = true;
	k_spin_unlock(&xs_client.lock, key);
...
int xs_watcher_unregister(struct xs_watcher *w)
{
	...
	key = k_spin_lock(&xs_client.lock);
	if (w->registered) {
		(void)sys_slist_find_and_remove(&xs_client.notify_list, &w->node);
		w->registered = false;
	}
	wait_for_callback =
		w->callback_running && (k_current_get() != k_work_queue_thread_get(&xs_client.workq));
	release_now = !w->callback_running;
	if (w->callback_running && !wait_for_callback) {
		w->release_after_callback = true;
	}
	k_spin_unlock(&xs_client.lock, key);
			    ...
			    uint32_t tx_id, k_timeout_t tout)
{
	...
	return execute_request(&xs_client, type, params, NULL, ARRAY_SIZE(params), buf, len, tx_id,
			       tout);
}
...
ssize_t xs_get_permissions_timeout(const char *path, struct xs_perm_entry *perms, size_t perms_num,
				   uint32_t tx_id, k_timeout_t tout)
{
	char *raw;
	size_t raw_len;
	...
	if (perms_num > (SIZE_MAX / XENSTORE_PERM_WIRE_ENTRY_MAX)) {
		return -EOVERFLOW;
	}

	raw_len = perms_num * XENSTORE_PERM_WIRE_ENTRY_MAX;
	...
	if (ret >= 0) {
		ret = xenstore_perm_parse_wire(raw, ret, perms, perms_num, &parsed_num);
		if (ret == 0) {
			ret = parsed_num;
		}
	}
			 ...
			 size_t len, uint32_t tx_id, k_timeout_t tout)
{
	...
	return execute_request(&xs_client, XS_WRITE, params, param_lens, ARRAY_SIZE(params), buf, len,
			       tx_id, tout);
}
			       ...
			       size_t perms_num, uint32_t tx_id, k_timeout_t tout)
{
	...
	if (perms_num > XENSTORE_PERM_MAX_ENTRIES) {
		return -E2BIG;
	}
	...
	const char *params[XENSTORE_PERM_MAX_ENTRIES + 1];
	char wire_perms[XENSTORE_PERM_MAX_ENTRIES][XENSTORE_PERM_WIRE_ENTRY_MAX];
	...
	for (size_t i = 0; i < perms_num; i++) {
		...
		ret = xenstore_perm_format_wire(wire_perms[i], sizeof(wire_perms[i]), &perms[i]);
	...
	ret = execute_request(&xs_client, XS_SET_PERMS, (const char *const *)params, NULL,
