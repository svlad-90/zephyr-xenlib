 ...
 * the backend-facing protocol path still uses ordinary Xen block requests with
 * up to four data grant pages per request and splits only larger transfers
 * internally. The current implementation discovers indirect, persistent-grant,
 * and multi-queue backend capabilities so callers can inspect them, but it does
 * not use those features for request submission.
 *
