...
/* Copy a XenStore path into frontend-owned storage with truncation checks. */
...
/* Build a child XenStore path under an already known base path. */
...
/* Accept only decimal digits before handing a value to libc parsing. */
static bool is_decimal_digit(char c)
{
	return (c >= '0') && (c <= '9');
}

/* Parse an unsigned XenStore numeric value without accepting signs or suffixes. */
...
/* Parse an unsigned 32-bit XenStore numeric value. */
...
/* Read a NUL-terminated XenStore string into a caller-provided scratch buffer. */
...
/* Read a named child from the backend XenStore node. */
...
/* Read and parse a required 32-bit backend XenStore feature or geometry node. */
...
/* Read and parse a required 64-bit backend XenStore geometry node. */
...
/* Read an optional boolean backend node, defaulting only when it is absent. */
...
/* Read an optional 32-bit backend node, defaulting only when it is absent. */
...
/* Write a string value to XenStore and normalize xs_write_timeout status. */
...
/* Format and write an unsigned integer XenStore value. */
...
/* Write a string child node below the frontend XenStore path. */
...
/* Write an integer child node below the frontend XenStore path. */
...
/* Poll a frontend child node that is expected to be supplied by the toolstack. */
...
int xen_blkfront_xenbus_publish_connected(struct xen_blkfront *front, char *buf, size_t len,
					  k_timeout_t timeout)
{
	return write_front_uint(front, "state", XenbusStateConnected, buf, len, timeout);
}

/* Poll backend state until it reaches either accepted close-handshake state. */
...
/* Discover read-only/read-write mode, defaulting missing mode to read-only. */
